...
// Які методи вимагають саме access_token
...
function formParams(params = {}, method = "") {
  ...
  // За можливості підставляємо ОБИДВА поля — це безпечно.
  if (TOKEN)  body.set("token", TOKEN);
  if (ACCESS) body.set("access_token", ACCESS);

  // Але додатково робимо валідацію: для методів, яким потрібен OAuth, мусить бути ACCESS.
  if (needsAccessToken(method) && !ACCESS) {
    throw new Error("Poster OAuth access token is missing (POSTER_ACCESS_TOKEN). Re-run OAuth.");
  }
  // Для методів, які покладаються на старий токен, перевіримо наявність хоча б чогось:
...
export async function posterCall(method, params = {}, preferPost = true) {
  ...
  // Спробуємо POST з коректними параметрами
  if (preferPost) {
    ...
    try { return JSON.parse(t); } catch { /* fallthrough to GET below */ }
  }

  // Фолбек на GET з такими ж параметрами
  const u = new URL(url);
...
export async function createSale({ reference, spotId, customer = {}, lines = [], total = 0 }) {
  // Poster приймає товари в іншому форматі:
